...
图片（含 HTTP(S) URL）会先上传到平台图片服务，以取得平台地址和实际尺寸。当前轮图片通过 `imageUrls` 发送；历史图片按网页协议保留在各自消息的多模态内容块中，合并文本放在图片块之前。不会把历史图片当作本轮新图片重新排列。当前轮文档先上传并通过同一会话的 `fileIds` 引用；历史文档使用平台解析结果恢复为该轮的文本内容。客户端应在每次请求中发送完整历史及附件，不依赖平台网页会话自动续接。

图片须小于 20 MiB，支持 PNG/JPEG/WEBP/GIF；文档须小于 10 MiB，TXT、文本型 PDF 和 DOCX 已完成实际上传解析验证。扫描 PDF、表格及其他格式由平台解析器决定效果。每次请求会重新处理所带附件，暂不提供附件缓存、`/v1/files` 存储或 `file_id` 引用接口。上游图片读取仍可能超时，代理会明确返回 502 或流内 error；生产聊天超时仍为 300 秒。多轮图片对照取得过正确结果，但重复观察仍出现超时，暂不保证所有模型、所有多图组合稳定可用。
...
- **令牌过期的恢复仍有缺口**：模型元数据和图片准备阶段遇到失效令牌时，实际观察到 HTTP 500、流中断或“平台未提供图片服务地址”，尚未统一恢复与正确传播错误。本轮重启服务恢复了登录状态，详见 Pi 验证报告。
...
- **图片上传令牌由代理自动获取**：图片走独立域名的图片服务，它不认用户登录令牌，只认网页前端写在前端脚本里的固定令牌。代理首次需要上传图片时从首页 `app.<hash>.js` 提取并缓存在进程内；上游改版导致提取失败时，图片输入会返回 502 并说明原因，文档与纯文本不受影响。
